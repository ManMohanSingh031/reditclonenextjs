            ...
            {comments.filter((comment) => !comment.replyToId).map((topLevelComment) => {
                ...
                return <div key={topLevelComment.id} className='flex flex-col'>
                    ...
                    {topLevelComment.replies.sort((a, b) => b.votes.length - a.votes.length).map((reply) => {
                        ...
                        const replyVotes = topLevelComment.votes.find((vote) => vote.userId === session?.user.id)
                        return <div className='ml-2 py-2 pl-4 border-l-2 border-zinc-200' key={reply.id}>
                            <PostComment postId={postId} currentVote={replyVotes} votesAmt={replyVotesAmt} comment={reply} />
                        </div>
                    })}
                </div>
